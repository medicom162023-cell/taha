...
export default function AssistanceAdmin() {
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-extrabold text-[#00466f]">إدارة طلبات المساعدات</h1><p className="text-sm text-slate-500">بيانات خاصة — للمخولين فقط</p></div><div className="flex gap-2"><a href="/homepage-admin" className="rounded-lg bg-[#00466f] px-4 py-2 font-bold text-white">إدارة الصفحة الرئيسية</a><button onClick={async()=>{await fetch('/api/admin/login',{method:'DELETE'});setAuthenticated(false);}} className="rounded-lg border px-4 py-2">تسجيل الخروج</button></div></div>
